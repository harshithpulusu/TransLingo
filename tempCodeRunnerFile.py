...
class EnhancedLanguageTranslatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AI-Powered Language Translator")
    ...
    def setup_settings_tab(self):
        ...
        self.voice_var = tk.StringVar(value="Alex")  # Default macOS voice
        voice_combo = ttk.Combobox(tts_frame, textvariable=self.voice_var)
        # Get available voices from system
        import subprocess
        result = subprocess.run(['say', '-v', '?'], capture_output=True, text=True)
        voices = [line.split()[0] for line in result.stdout.splitlines()]
    ...
    def _speak_text(self, text):
        ...
        try:
            import subprocess
            subprocess.run(['say', '-v', self.voice_var.get(), '-r', str(self.tts_rate_var.get()), text])
        except Exception as e:
            print(f"TTS error: {e}")
